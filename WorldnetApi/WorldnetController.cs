...
public class WorldnetController(string baseUrl, string apiKey)
{
    ...
    private WorldnetToken? _token = null;
    private readonly ILogger _log = Log.ForContext<WorldnetController>();

    public async Task<PostPaymentInstructionsResponse> PostPaymentInstructions(string serialNumber, PostPaymentInstructionsBody body)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Post, url)
        {
            ...
            Content = new StringContent(body.Serialize())
        };
    ...
    public async Task<SearchTransactionsJson> SearchTransaction(SearchTransactionsQuery query)
    {
        await RefreshTokenIfNeeded();
        ...
        {
            var json = await response.Content.ReadAsStringAsync();
            return SearchTransactionsJson.FromJson(json);
        }
        else
        {
            var json = await response.Content.ReadAsStringAsync();
    ...
    {
        if (_token == null || _token.ExpiresIn.TotalMinutes < 10)
        {
            await RefreshToken();
        }
    }
